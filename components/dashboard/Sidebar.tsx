...
export default function Sidebar() {
    ...
    return (
        <>
            ...
            {/* Sidebar Container */}
            <aside
                className={`fixed md:static inset-y-0 left-0 z-50 bg-white border-r border-gray-200 transition-all duration-300 ease-in-out flex flex-col 
        ...
        `}
            >
                {/* Desktop Header / Logo */}
                <div className="hidden md:flex items-center justify-between h-16 px-4 border-b border-gray-100">
                    {!isCollapsed && (
                        <div className="flex items-center gap-2 font-bold text-xl text-emerald-600 transition-opacity duration-300">
                            <Heart className="w-6 h-6 fill-current" />
                            <span>VetCare</span>
                        </div>
                    )}
                    {isCollapsed && (
                        <Heart className="w-6 h-6 text-emerald-600 fill-current mx-auto" />
                    )}
                    <Button
                        variant="ghost"
                        size="icon"
                    ...
                    <div className="p-4 border-b border-gray-100 hidden md:block">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="overflow-hidden">
                                ...
                                <p className="text-xs text-gray-500 capitalize">{user.role === 'veterinarian' ? 'Veterinário' : user.role}</p>
                            </div>
                        </div>
                    </div>
                )}
                ...
                `}
                                ...
                                onClick={() => setIsMobileOpen(false)}
                            >
                                <Icon className={`w-5 h-5 flex-shrink-0 ${isActive ? "text-emerald-600" : "text-slate-500 group-hover:text-emerald-600"}`} />
                                {!isCollapsed && (
                ...
                <div className="p-4 border-t border-gray-100 space-y-2">
                    {!isCollapsed && (
                        <Link href="/notifications" className="flex items-center gap-3 px-3 py-2 text-sm text-slate-600 hover:text-emerald-600 rounded-lg hover:bg-slate-50">
                            <Bell className="w-5 h-5" />
                            <span>Notificações</span>
                        </Link>
                    )}
                    {isCollapsed && (
                        <Link href="/notifications" className="flex justify-center p-2 text-slate-600 hover:text-emerald-600 rounded-lg hover:bg-slate-50" title="Notificações">
                            <Bell className="w-5 h-5" />
                        </Link>
                    )}
